...
const IntensityGraphs = ({ jsonData = [] }) => {
    ...
    useEffect(() => {
        if (Array.isArray(jsonData) && jsonData.length > 0) {
            ...
            const intensityByCountry = filteredData.reduce((acc, item) => {
                if (item.country in acc) {
                    acc[item.country].count++;
                    acc[item.country].totalIntensity += item.intensity;
                } else {
                    acc[item.country] = {
                        count: 1,
            ...
            const averageIntensityData = Object.entries(intensityByCountry).map(([country, { count, totalIntensity }]) => ({
                country,
                averageIntensity: totalIntensity / count,
            }));

            // Set data state
            setData(averageIntensityData);
        }
    ...
    const drawBarChart = (data) => {
        ...
        const y = d3.scaleLinear()
            .domain([0, d3.max(data, d => d.averageIntensity)])
            .nice()
            .range([height, 0]);
        ...
        svg.selectAll(".bar")
            ...
            .on("mouseover", function (event, d) {
                ...
                d3.select(this.parentNode).select(`#text-${d.country.replace(/\s+/g, '-')}`).style("display", "block");
            })
            .on("mouseout", function (event, d) {
                // Hide the text element when not hovering
                d3.select(this.parentNode).select(`#text-${d.country.replace(/\s+/g, '-')}`).style("display", "none");
            })
            .transition()
            .duration(1000)
            .attr("d", d => {
                const xPos = x(d.country);
                const yPos = y(d.averageIntensity);
        ...
        svg.selectAll(".bar-text")
            .data(data)
            .enter().append("text")
            .attr("id", d => `text-${d.country.replace(/\s+/g, '-')}`)
            .attr("x", d => x(d.country) + x.bandwidth() / 2)
            .attr("y", d => y(d.averageIntensity) - 5)
            .attr("text-anchor", "middle")
            .style("fill", "#a6a6a6")
            .style("display", "none") // Hide text initially
            .text(d => `${Number.isInteger(d.averageIntensity) ? d.averageIntensity : d.averageIntensity.toFixed(2)}`);
    };
    ...
    return (
        <Box>
            <Box
                ...
                fontSize={18}
                fontWeight={500}
            >Intensity Vs Country Graph:-</Box>
            <Box
                display='flex'
                flexDirection={{ base: 'column', md: 'row' }}
            >
                <Select mr={2} placeholder="Select End Year"
                    // _placeholder={{ color: 'rgba(152, 162, 179, 1)' }}
                    sx={{
                        color: '#8c8c8c',
                    }}
