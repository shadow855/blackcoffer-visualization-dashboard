...
const RegionGraphs = ({ jsonData = [] }) => {
    ...
    useEffect(() => {
        if (Array.isArray(jsonData) && jsonData.length > 0) {
            ...
            const intensityByRegion = filteredData.reduce((acc, item) => {
                if (item.region in acc) {
            ...
            const averageIntensityData = Object.entries(intensityByRegion).map(([region, { count, totalIntensity }]) => ({
                region,
    ...
    const drawBarChart = (data) => {
        ...
        svg.selectAll(".bar")
            ...
            .on("mouseover", function (event, d) {
                ...
                d3.select(this.parentNode).select(`#text-${d.region.replace(/\s+/g, '-')}`).style("display", "block");
            })
            .on("mouseout", function (event, d) {
                // Hide the text element when not hovering
                d3.select(this.parentNode).select(`#text-${d.region.replace(/\s+/g, '-')}`).style("display", "none");
            })
            .transition()
            .duration(1000)
            .attr("d", d => {
        ...
        svg.selectAll(".bar-text")
            .data(data)
            .enter().append("text")
            .attr("id", d => `text-${d.region.replace(/\s+/g, '-')}`)
